...
def main() -> int:
    ...
    pd.DataFrame([
        # claim, earliest source we can trace, source class, applies-to, status
        {"claim": "Data collected from IoT devices in Tripura, India via REST APIs, 52 locations, 15-min updates, from Nov 2020",
         "earliest_source": "Putchala et al. 2022 (S11), Sec. 4 (PRIMARY: dataset authors' platform)",
         "source_class": "PRIMARY",
         "applies_to": "KernelSphere live platform (Nov 2020+)", "applies_to_kaggle_export": False,
         "note": "Kaggle export window is 2019-06-25..2020-04-14, published 2020-05-25 — BEFORE the paper's collection start; the paper describes the platform, not this export's window"},
        {"claim": "Column schema (OTI, WTI, ATI, OLI, OTI_A, OTI_T, MOG_A, VL*, IL*, KW, KVA, ...)",
         "earliest_source": "Kaggle dataset page (provider) + Putchala Table 2 (identical names)",
         "source_class": "PRIMARY",
         "applies_to": "Kaggle export and live platform", "applies_to_kaggle_export": True,
         "note": "Schema match establishes platform kinship, not entity identity"},
        {"claim": "OTI > 65 treated as trip threshold; OTI_T derived as binary failure-prediction target",
         "earliest_source": "Putchala et al. 2022 (S11), Sec. 4.2 (their modelling choice on live stream)",
         "source_class": "PRIMARY",
         "applies_to": "Putchala modelling pipeline", "applies_to_kaggle_export": False,
         "note": "In the Kaggle export OTI_T aligns exactly with OTI>=236 — a different threshold; semantics of OTI_T (indication vs command) remain unexplained"},
        {"claim": "Transformer is 1500 kVA, 11/0.4 kV",
         "earliest_source": "Energies 15(21):7981 (S7), Sec. 5.1 (their own Sharjah system)",
         "source_class": "SOURCE-ADJACENT",
         "applies_to": "AUS/Sharjah PSCAD test system", "applies_to_kaggle_export": False,
         "note": "No primary source connects this rating to the Kaggle export; 'similar transformers sharing load capacity' (Energies) is not a rating transfer"},
        {"claim": "OTT shuts off electrical flow when oil temperature is high",
         "earliest_source": "Energies 15(21):7981 (S7), Sec. 7.1 (generic description)",
         "source_class": "SOURCE-ADJACENT",
         "applies_to": "generic OTT description", "applies_to_kaggle_export": False,
         "note": "In the Kaggle export, voltage/current continue through OTI_T=1 windows — electrical continuity contradicts a breaker-open interpretation for THIS data; OTI_T semantics stay open"},
        {"claim": "Overheat marker near 90 (C-scale) for oil temperature",
         "earliest_source": "Energies 15(21):7981 (S7), Sec. 8 (anomaly detection on their system)",
         "source_class": "SOURCE-ADJACENT",
         "applies_to": "their GRU/IF monitoring", "applies_to_kaggle_export": False,
         "note": "Kaggle excursions reach 236-250 OTI-units — far above any documented operating band; OTI unit unconfirmed"},
        {"claim": "Dataset is about one distribution transformer",
         "earliest_source": "NONE FOUND — appears only as community/project assumption",
         "source_class": "SECONDARY",
         "applies_to": "—", "applies_to_kaggle_export": False,
         "note": "No primary source states the export's entity scope; provider platform covers 52 locations; export has no device/location column"},
        {"claim": "Energies 2022 experiments used the Kaggle dataset sreshta140/ai-transformer-monitoring, accessed 20 June 2022",
         "earliest_source": "Energies 15(21):7981 (S7), Data Availability Statement (verbatim, verified in full text)",
         "source_class": "SOURCE-ADJACENT",
         "applies_to": "the audited dataset (same upload)", "applies_to_kaggle_export": True,
         "note": "The audited archive IS sreshta140/ai-transformer-monitoring v1 (single version, 'Initial release', 2020-05-25; official Kaggle API download, hash-pinned). Energies accessed that URL on 2022-06-20; only version 1 exists, so the same v1 archive"},
        {"claim": "The audited dataset was uploaded by the first author of Putchala et al. 2022 (Sreshta R. Putchala, CBIT Hyderabad)",
         "earliest_source": "Kaggle dataset owner 'Sreshta Putchala' (dataset 'Distributed Transformer Monitoring', v1 2020-05-25) + Springer author list 'Putchala, S.R., Kotha, R., Guda, V., Ramadevi, Y.' (S11)",
         "source_class": "PRIMARY (authors' own upload — this is the audited copy)",
         "applies_to": "audited dataset", "applies_to_kaggle_export": True,
         "note": "Links the audited export to the paper's research group and, via the paper's Sec. 4, to the KernelSphere platform — but the paper's stream starts Nov 2020, AFTER the export window; platform attribution of THIS window remains an inference"},
        {"claim": "A byte-identical mirror exists (pythonafroz/transformer-fault-analysis, v3): same 5 CSVs with Overview.csv renamed Alarm.csv",
         "earliest_source": "Phase-1 mirror verification (S9): sha256 match on all five files",
         "source_class": "SECONDARY (re-upload)",
         "applies_to": "mirror only", "applies_to_kaggle_export": True,
         "note": "Confirms the files circulate as re-uploads; no new provenance information in the mirror"},
