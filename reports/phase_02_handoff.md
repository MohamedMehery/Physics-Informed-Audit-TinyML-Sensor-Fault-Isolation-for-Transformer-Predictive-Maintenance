...
v1 ("Distributed Transformer Monitoring", single version, 2020-05-25; official
API download; SHA-256-pinned in `provenance/dataset_manifest.json`). Raw files
gitignored, byte-preserved, and hash-verified before and after every run.
...
1. **Full-text source examination** (not abstracts) with source-role
   ...
   for all relied-upon sections including the Data Availability Statement
   and conclusions; manufacturer/utility envelope gathered from official
   sources only (ETT, BSES Delhi, Bharat Bijlee, IS 1180 scope). Short
   section-referenced quotes only; no copyrighted full text committed.
2. **Lineage triangle closed:** the audited archive was uploaded by
   "Sreshta Putchala" — the first author of Putchala et al. 2022 (Springer
   author list verified) — and the Energies study used this same dataset
   (its Data Availability Statement names the URL, accessed 2022-06-20;
   only v1 exists). The paper's platform statement (KernelSphere, Tripura,
   52 locations, REST APIs, 15-min, from Nov 2020) describes a *later*
   window than the export (2019-06-25…2020-04-14); platform attribution of
   the export window is an inference (uploader + identical schema + cadence
   + overlapping operating ranges), not a primary statement.
3. **Repeated-record analysis** (never "duplicates"; never averaging
   conflicting OTI/flags): multiplicity, cross-file sharing, raw-line
   adjacency, conflict magnitudes, monthly concentration, occurrence-stream
   diagnostics, branch-continuity parity, high-OTI intersection.
4. **Policy sensitivity** P1 preserve-all / P2 first / P3 last /
   P4 identical-only: excursion findings invariant; Phase-1 `mean` policy
   removed from all flag statistics (code review `phase_01_code_review.md`;
   no Phase-1 headline number changed).
5. **Variable semantics** per column: OTI in "OTI units" (unconfirmed), WTI
   binary, OLI unit unknown, OTI_T⟺OTI≥236 exact but semantics open; the
   near-250 band is never called an ADC rail; representation hypotheses
   listed with falsification tests.
6. **Asset parameter bounds:** identity parameters (#transformers,
   #devices, #feeders, rating, oil volume) all UNKNOWN; observed loading
   (142.9 kVA max) establishes nothing about rating; official envelope used
   only as market context; rating unbounded above.
7. **Conditional physics** (after entity/unit/source reviews; every result
   ...
   (τ with log-domain guards and ambient/Δt sensitivity). No IEC ODE, no
   ML, no edge detector, no hardware root cause.
8. **Independent verification:** stdlib-only
   `scripts/independent_raw_verification.py` recomputes row counts, hashes,
   ranges, repeated counts, identical-vs-conflicting, OTI min/max, #≥236,
   empty interval, OTI_T⟺OTI, actual-Δt rates, and raw-line locations —
   56/56 checks agree; exits non-zero on disagreement.

## Headline results

- **Lineage:** dataset ↔ Putchala et al. 2022 ↔ Energies 2022 verified as
  one lineage (same archive). The Energies 1500 kVA / 11→0.4 kV description
  is their own Sharjah system, not this dataset. The paper's OTI>65 trip
  band belongs to their live stream; the export's OTI_T aligns with
  OTI ≥ 236 instead.
- **Required statement (entity unresolved):** *The published dataset does
  ...
  H3 (multi-asset) and H4 (multi-feeder) find no support in any test.
...
critical-mass math, τ domain guards, sensitivity, and verifier agreement.
Environment: Python 3.13.14, pandas 2.2.3, numpy 2.3.5, matplotlib 3.10.9.
Commands: `run_data_audit.py`, `run_phase2_analysis.py`,
`independent_raw_verification.py`, `pytest` — all with pre/post provenance
gates (PASS).
...
`reports/generated/` (timestamp_multiplicity, repeated_record_conflicts*,
high_oti_duplicate_intersection, duplicate_policy_sensitivity,
latent_stream_diagnostics, variable_semantics, asset_parameter_evidence,
physics_sensitivity, apparent_tau_per_transition, oti_transitions_phase2,
alternative_explanations, source_claim_matrix, bibliography_register,
manufacturer_spec_envelope, phase_02_summary.json) ·
`reports/figures/physics_bounds_*.png` · registers updated
(C01–C36, A01–A27, S1–S16).

## Standing constraints preserved

Repeated records never silently discarded; no numeric probabilities;
"OTI units" not °C; near-250 never "ADC rail"; observed loading ≠ nameplate;
