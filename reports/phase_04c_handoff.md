...
(https://doi.org/10.5281/zenodo.22904790) was verified against the
Zenodo record API before propagation (record 22904790: title
"Transformer Telemetry Integrity Audit", version v0.1.0, creator
Mehery, Mohamed, license Apache-2.0, created 2026-09-22). No
experiments, no scientific-content changes, nothing sent.
...
generated from the updated CITATION.cff via cffconvert — the tool's
placeholder key renamed to `mehery2026transformer` and a version line
added); `CITATION.cff` (+ `doi`, + `url`; cffconvert-validated, schema
...
`paper/technical_report_v0_1.md` and its cited artifacts; no forbidden
phrasing, no accusation, no invalidation of prior work; Phase-3R
caveats preserved (operationally defined events, no field-confirmed
labels, post-hoc internal validation only). Word counts verified
programmatically (see below). KernelSphere and mirror-uploader drafts
remain stored unfinalized in `docs/outreach_requests.md`.
...
FORBIDDEN list found 0 offending lines (these files are outside the
test's scan scope by design, since they quote dataset findings).
