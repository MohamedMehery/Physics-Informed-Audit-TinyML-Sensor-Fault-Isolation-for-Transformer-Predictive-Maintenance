...
| A21 | 100 % of electrical power converts to heat in the measured oil body | Conditional physics §A | Extreme upper bound only | Realistic loss fraction is far smaller; every required-power figure grows and every feasible mass shrinks if relaxed | **EXPLICIT EXTREME BOUND** |
| A22 | Specific heat c = 1.8–2.1 kJ/(kg·K) | Conditional physics §A | Mineral-oil typical band | Oil type unknown | **SCENARIO LABEL** |
| A23 | OTI units behave like temperature differences for ΔT and τ | Conditional physics §A/B | Unit unconfirmed | τ is offset-invariant; an unknown scale factor scales derived masses/powers linearly | **CONDITIONAL** |
| A24 | Power during transitions is bounded by export-observed loads (local ±30-min max 33–103 kW; global max 142.1 kW) unless explicitly parameterized (500/1500 kW) | Conditional physics §A | Observed-power bound | Sampling cannot exclude unobserved sub-interval electrical events — stated limitation, not a claim | **OBSERVED-POWER BOUND + STATED LIMIT** |
| A25 | ATI channel is a usable ambient proxy (±5-unit sensitivity applied) | Conditional physics §B | Unit unconfirmed (°C-consistent) | ±5 shifts τ by up to ~2× at the smallest Δt | **PROXY** |
| A26 | Repeated-timestamp records originate from ingestion/export re-transmission (H2) | Phase-2 interpretation of repeats | Adjacency, cross-file sync, tiny conflicts, deployment-month concentration | Unconfirmed without provider logs; stated as "most consistent," never as fact | **QUALITATIVE BEST-FIT, UNCONFIRMED** |
| A27 | Entity scope (one transformer vs many) is unresolvable from the published export | Phase-2 entity analysis | No device column; no primary scope statement; 52-location platform | Required statement C32 stands until provider answers outreach | **STANDING UNKNOWN** |
